...
func _on_function_text_changed(new_text):
	if "#Todo:" in new_text or "#todo:" in new_text or "Todo:" in new_text or "todo" in new_text or "Todo" in new_text or "TODO" in new_text :
		$Function.modulate = "#50fa7b"
	if "#Hack:" in new_text or "#hack:" in new_text or "Hack:" in new_text or "hack" in new_text or "Hack" in new_text or "HACK" in new_text:
		$Function.modulate = "#ffb86c"
	if "#Fix:" in new_text or "#fix:" in new_text or "Fix:" in new_text or "fix" in new_text or "Fix" in new_text or "FIX" in new_text:
		$Function.modulate = "#ff5555"


func _on_task_text_changed(new_text):
...
func _on_file_text_submitted(new_text):
	var file
	if new_text:
		file = load(new_text)
	if file:
